...
const scrape = async (startRatio, endRatio) => {
  ...
  let courseCodes = await page.evaluate(() => {
    ...
    let allCoursesDiv = document
      .querySelectorAll('div.selectize-dropdown-content')[3]
  ...
  for (let i = start; i < end; i++) {
    ...
    await page.goto(`https://student.utm.utoronto.ca/timetable?course=${courseCode}`, {
      waitUntil: 'networkidle0',
    });
    ...
    let coursesRawInfo = await page.evaluate(() => {
      ...
      for (let rawCourseInfo of allCoursesRawInfo) {
        ...
        let courseInfo = {
          ...
          rawBreadths:
            rawCourseInfo.querySelector('span > h4 > b') != null
              ? rawCourseInfo.querySelector('span > h4 > b').innerText
              : 0,
        ...
        for (let currSectionDiv of allSectionDivs) {
          ...
          for (let rawLocations of rawInfo[10].querySelectorAll('span')) {
            ...
            allLocations.push(
              rawLocations.innerText.length > 2 ? rawLocations.innerText : '',
            );
          }
    ...
    for (let currCourseRawInfo of coursesRawInfo) {
      ...
      for (let rawSectionInfo of currCourseRawInfo.rawMeetingSections) {
        ...
        if (
          rawSectionInfo[11] == 'Online Asynchronous' ||
          rawSectionInfo[12].includes('Closed')
        ) {
          continue;
        }

        currMeetingSection.sectionCode = `${
          rawSectionInfo[1][0]
        }${rawSectionInfo[1].slice(3, rawSectionInfo[1].length)}`;
        currMeetingSection.instructors = formatInstructor(rawSectionInfo[2]);
        currMeetingSection.times = formatTimes(
          rawSectionInfo[8],
          rawSectionInfo[9],
          rawSectionInfo[7],
          rawSectionInfo[10],
