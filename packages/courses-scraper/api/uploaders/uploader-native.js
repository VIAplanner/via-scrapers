...
fs.readdirSync(outputPath).forEach((fileName) => {
  ...
  allSearchbarValues.values.push({
    courseCode: currCourseData.courseCode,
    name: currCourseData.name,
  });
});
...
MongoClient.connect(url, { useUnifiedTopology: true }, async (err, db) => {
  ...
  try {
    await Promise.all([
      db.db('data').collection('courses').insertMany(allCourseData),
      db.db('data').collection('searchbars').insertOne(allSearchbarValues),
    ]);
  } catch (err) {
    db.close();
    spinner.stop();
